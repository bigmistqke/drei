...
export const PlaneSlider: Component<{ dir1: THREE.Vector3; dir2: THREE.Vector3; axis: 0 | 1 | 2 }> = (props) => {
  const {
    translation,
    translationLimits,
    annotations,
  ...
  const onPointerDown = (e: ThreeEvent<PointerEvent>) => {
    if (annotations) {
      divRef.innerText = `${translation[(props.axis + 1) % 3].toFixed(2)}, ${translation[(props.axis + 2) % 3].toFixed(
        2
      )}`
  ...
  return (
    <T.Group ref={objRef} matrix={matrixL()} matrixAutoUpdate={false}>
      {annotations && (
        <Html position={[0, 0, 0]}>
          <div
            style={{
